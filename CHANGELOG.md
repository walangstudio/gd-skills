...
- **`/verify-project`** — a user-runnable post-generation checker that gates a game on completeness (all promised systems present, no stubs/dead refs), wiring, registry consistency, **configurability** (tunables externalized, no magic numbers), and a working core loop; prints a PASS/WARN/FAIL checklist and proposes fixes. It runs the same flow builders already run via `guides/autonomous-validation.md`
