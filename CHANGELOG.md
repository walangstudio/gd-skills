...
- Runnable web samples: coin-collector, snake, breakout, platformer, tower-defense, match3, shooter, asteroids, g2048, rpg, racing, roguelike, survival, farming, horror (under `samples/web/`) — dependency-free HTML5 canvas games, each with logic split from rendering and headless unit tests (`node test.js`); `scripts/test-samples.sh` runs all 15 in CI. Together they cover every genre template
- **`guides/game-feel.md`** — the juice catalog: every feel technique (coyote time, jump buffer, variable jump, asymmetric gravity, screen shake, hit stop, knockback, damage/screen flash, squash & stretch, anticipation, follow-through/overshoot, camera kick, slow-mo, vibration, particles, layered SFX, damage numbers, input buffering) with its purpose, a starting value, and a **per-engine API map** (Godot/Unity/Unreal/Roblox/Web/Defold — Cinemachine Impulse, `UCameraShakeBase`, `TweenService`, `Highlight`, etc.). Wired into `add-game-feel` + `game-feel-specialist`; every value is flagged as config per the new tuning rule
- **`/verify-project`** — a user-runnable post-generation checker that gates a game on completeness (all promised systems present incl. **audio** and **baseline game feel / polish**, no stubs/dead refs), wiring, registry consistency, **configurability** (tunables externalized, no magic numbers), and a working core loop; prints a PASS/WARN/FAIL checklist and proposes fixes. It runs the same flow builders already run via `guides/autonomous-validation.md`. Audio and polish are now treated as part of "done", verified — not just built and assumed
- **`rules/configuration-and-tuning.md`** — universal no-magic-numbers rule: every tunable (speed, health, damage, cooldowns, spawn rates, costs, level data) must be an engine-idiomatic exported property / config in one discoverable place, never a literal buried in logic. The autonomous-validation loop now enforces it (new Completeness + Configurability steps + report lines)
- Platformer template: **2.5D cinematic platformer (REPLACED style)** recipe — 2D gameplay on a fixed plane inside a real 3D world, with the per-engine setup (Godot `CharacterBody3D` plane-lock + `Sprite3D` + volumetric fog/bloom/DOF, Unity URP/HDRP Volume, Unreal Paper2D + volumetric fog, Three.js bloom/DOF/god-ray passes, Roblox `Lighting` FX), a Perspective customization option, and the reference named in the `/create-platformer` menu
- RPG template: **HD-2D / 2.5D JRPG (Octopath Traveler style)** as a first-class sub-genre and perspective option — billboarded 2D sprites in a real 3D scene with depth-of-field/tilt-shift, bloom, and color grading, plus a Break/Boost combat note; includes the per-engine rendering recipe (Godot `Sprite3D` + `CameraAttributesPractical` DOF, Unity URP Volume, Unreal Paper2D + post-process, Three.js `BokehPass`/`UnrealBloomPass`, Roblox `Lighting` FX) and surfaced in the `/create-rpg` perspective menu
